...
Vamos aqui interpretar o significado do programa dual. \\ 
Vamos chamar as variáveis $v$ de orçamento e as variáveis $w$ de contribuição. \\Uma instalação recebe contribuições dos clientes para pagar pela sua abertura. Uma vez que as contribuições são suficientes para a sua abertura, ela não precisa receber mais. Isso está explicito na restrição~$\ref{D2}$.\\
O orçamento de um cliente será utilizado para pagar sua associação a uma instalação e sua contribuição para abertura dela. Isso está explicito na restrição~\ref{D3}.\\
Para entender como as variáveis primais e duais se relacionam vamos supor a existência de uma solução ótima inteira para o primal e analisar as folgas complementares. Em uma solução inteira do primal, as variáveis $x$ e $y$ adotam apenas os valores das restrições do programa inteiro.
...
Vamos construir uma solução $(\bar{v},\bar{w})$ maximal para o dual, ou seja, uma solução que não podemos aumentar o valor das variáveis $v$ e então construir um conjunto $w^*$ que, junto com $v^*$, seja uma solução viável.\\
Iremos alterar a definição de vizinhança para que cubra também as instalações.
\begin{definition}
...
\end{definition}
Além disso, vamos precisar também de outra definição.
\begin{definition}
    Seja $(v,w)$ uma solução viável do dual. Dizemos que um cliente $j \in D$ \emph{contribui} para uma instalação $i$ se $w_{ij} > 0$.
\end{definition}

Dadas variáveis duais $v$, conseguimos derivar um conjunto $w$ viável, se ele existir, se adotarmos $w_{ij} = \max(0,v_j - c_{ij})$. Assim, se $w_{ij} >0$, então $j \in N(i)$, uma vez que $w_{ij} > 0$ implica $v_j > c_{ij}$ e $v_j = c_{ij} + w_{ij}$. Veremos isso será válido na nossa solução.\\
Seja $T \subseteq F$ tal que $T = \{i \in F: \sum_{j \in D} w_{ij} = f_i\}$. Em uma solução maximal $(\bar v, \bar w)$ do dual, todo cliente está na vizinhança de uma instalação em $T$. Caso contrário, conseguimos aumentar o valor das variáveis e encontrar uma solução viável com custo melhor para o dual. Seja $i \in T$ temos que,
\begin{equation} 
    \label{inst_sol}
    f_i + \sum_{j \in N(i)} c_{ij} = \sum_{j \in D} \bar w_{ij} + \sum_{j \in N(i)} c_{ij} = \sum_{j \in N(i)} (\bar w_{ij} + c_{ij})
\end{equation}
em que a primeira igualdade vale pela definição de $T$ e a segunda vale pela definição de vizinhança. \\
Como cada cliente está na vizinhança de uma instalação que pertence a $T$, abrir todas as instalações em $T$ seria suficiente, mas estaremos, provavelmente, sendo redundantes. Podemos então escolher um conjunto $T' \subseteq T$ tal que cada cliente esteja na vizinhança de no máximo uma instalação, mas garantindo que um cliente que não esteja na vizinhança de nenhuma instalação de $T'$ esteja próxima a alguma delas. \\
\begin{algorithm}
    \begin{algorithmic}[1]
        \Function{JV}{{F,D,c,f}}
        \State $v \gets 0$
        ...
        \If{algum $j\in S$ passe a pertencer a vizinhança de alguma instalação em $T$}
