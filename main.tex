...
\begin{definition}
    Seja $G = (V,E)$ um grafo. Um conjunto $S \subseteq V$ é uma \emph{cobertura por vértices} de $G$ se para toda aresta $e \in E$ pelo menos um extremo de $e$ pertencem a $S$.
\end{definition}
...
\begin{proof}
    ...
    Seja $I(G,c)$ uma instância do problema da cobertura por vértices em que $G = (V,E)$. Vamos criar uma instância $I'(F,D,c',f)$ do problema de localização de instalações a partir de $I$.\\
    ...
    A reposta ótima será a menor quantidade de instalações que precisamos abrir até que todos os clientes estejam atribuídos a instalações em que o seu custo de atribuição seja 0. Se um cliente está atribuído a uma instalação cujo custo de atribuição é 2, podemos diminuir o custo total abrindo uma instalação que o custo de sua atribuição seja 0, uma vez que abrir a instalação custaria 1 e diminuiríamos o custo de atribuição do cliente de 2 para 0. Sabemos que essa instalação existe, pois cada cliente contém duas instalações com custo de atribuição 0. \\
